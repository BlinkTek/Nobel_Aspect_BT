...
export default function App() {
  ...
  const handleSubmit = async (onClose) => {
    try {
      const response = await axios.post("http://localhost:8000/service/create", modalData);
      console.log(response.data);
    } catch (err) {
      console.log(err.response?.data?.message || "Failed to add service. Please try again later.");
    }
    onClose();
    fetchData();
  };
            ...
            <div className="relative flex justify-end items-center gap-2">
              <Dropdown className="bg-background border-1 border-default-200">
                ...
                <DropdownMenu className="text-black">
                  ...
                  <DropdownItem onPress={() => deleteService(user)} className="text-red-500">
                    Delete
                  </DropdownItem>
                </DropdownMenu>
              </Dropdown>
