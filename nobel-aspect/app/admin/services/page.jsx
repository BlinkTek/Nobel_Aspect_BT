...
export default function App() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}api/service/list`);
  ...
  const handleSubmit = async (onClose) => {
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}api/service/create`,
        formData,
        {
          headers: {
  ...
  const handleEditData = async (onClose) => {
    try {
      const response = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}api/service/edit/${modalData._id}`, modalData);
    } catch (err) {
  ...
  const deleteService = useCallback(async (user) => {
    try {
      const response = await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}api/service/delete/${user._id}`);
    } catch (err) {
